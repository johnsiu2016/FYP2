...
export function calculateECGArray_BAK(baseECGArray, HR) {
  const baseDataLength = baseECGArray.length;
  const HRDataLength = HR / 60 * baseDataLength;
  ...
  const execFunc = () => {
    previousPart = baseECGArray.slice(splitPoint);
    extraDataLength = HRDataLength - previousPart.length;
    splitPoint = (extraDataLength - 1) + 1;
    newPart = baseECGArray.slice(0, splitPoint);

    if (splitPoint >= baseDataLength) splitPoint = 0;

    console.log(previousPart.length);
    console.log(splitPoint);
    console.log(newPart.length);
    console.log((previousPart.concat(newPart)).length);

    return interpolateArray(previousPart.concat(newPart), 240);
  };
